...
| TX-005 | Daily budget state-aware arithmetic | FIXED | `0238e1b` windows `confirmed/reverted` by `COALESCE(finalized_at,created_at)` (late-finalization) and keeps `submitted/pending/unknown` reserved while `finalized_at IS NULL` (old-unknown). `transactionEngine.integration.test.js` `rollingSpendWei` 2/2; boundary/replacement/missing-revert-cost cases remain for VERIFIED |
...
| TX-024 | Dashboard batch duplicate/unbounded wallets | FIXED | `fdeab15` `src/dashboard/api.js:270-275` rejects empty/duplicate/>100 before `prepareMint`; `tests/dashboard*.test.js` + manual batch 5/5 |
...
| TX-027 | Dashboard false-success for reverted/replaced intent | FIXED | `fdeab15` `src/dashboard/api.js:282` `if(result.state!=='confirmed')` → `failed`; `src/server.js:760` `wallet.minted` only on `success`. `tests/scheduledFailureSurfaces.test.js` 3/3 |
...
| TX-030 | Replacement fee increase omitted from spend reservation | FIXED | `bda1dac` `bumper.js:98` computes `newEstimatedCostWei = value + gasLimit*newFee` and `intentRepository.js:119` `attachBump(...estimatedCostWei)` sets `estimated_cost_wei` before broadcast (atomic with bump). `tests/bumper.test.js` 5/5 |
