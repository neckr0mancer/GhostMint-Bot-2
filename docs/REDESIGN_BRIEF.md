...
| **O7** | **`GET /api/profile/limits` does not exist.** Would return `{maxTransactionValueWei, dailySpendingBudgetWei, spentTodayWei, gasCeilingGwei}` and restore the daily-budget meter cut by D9 | After the redesign, and **only after** `rollingSpendWei`'s under-count is fixed. Logged in `docs/WORKLIST.md` |
