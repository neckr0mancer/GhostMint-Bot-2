...
function createDashboardAuthService({identity,repository,now=()=>Date.now(),sessionTtlMs=SESSION_TTL_MS}) {
  ...
  async function authenticate(cookieHeader){const token=parseCookies(cookieHeader)[SESSION_COOKIE];return token?repository.resolve(hash(token),now()):null;}
  ...
  return {authenticate,login,verifyCsrf,revoke:session=>repository.revoke(session.sessionId),revokeAll:session=>repository.revokeAll(session.userId),
