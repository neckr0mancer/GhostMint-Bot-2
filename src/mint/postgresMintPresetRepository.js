  ...
  return {
    async save(preset) {
      const result = await pool.query(`INSERT INTO mint_presets
        (user_id,name,contract_address,chain,method_signature,abi_fragment,arguments,value_wei,proof_source_url)
        ...
        ON CONFLICT (user_id,(LOWER(name))) DO UPDATE SET
          ...
          proof_source_url=EXCLUDED.proof_source_url,updated_at=NOW() RETURNING *`,
