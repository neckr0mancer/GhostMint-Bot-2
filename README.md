...
GhostMint is an Express, Telegram, Discord, and browser-dashboard service for managing EVM wallets, scheduling NFT mints, submitting manual and batch mints, and watching target wallets for configurable copy-mint activity.

> **Project status:** active prototype. PostgreSQL persistence, versioned envelope encryption, multi-user identity, transaction safety, flexible mint calls, durable scheduling, and confirmed-default plus opt-in pending-mempool copy watching are implemented. Production acceptance remains incomplete. Use disposable test wallets only.
...
Telegram `/link` creates a cryptographically random code that expires after five minutes and can be consumed once. An authenticated dashboard session can generate or refresh the same kind of code from Account → Linked platforms; refreshing immediately invalidates the previous unused code. Discord's `/link code:<value>` (and its Settings menu equivalent) can only consume one, never generate one -- `code` is a required option on Discord's `/link`, and there is no "generate" path on Discord itself. Run `/link` in Telegram or generate a code from an existing dashboard session, then use `/link code:<value>` in Discord to attach Discord directly to that same internal user. Link-code consumption occurs before Discord's first-seen auto-creation path, avoiding a duplicate identity. A code can also be entered at `/dashboard/` to create a revocable browser session for that existing internal user. Username + security-password dashboard login is also available when the account has explicitly configured both credentials.

### Discord bot

Configure `DISCORD_BOT_TOKEN` and `DISCORD_APPLICATION_ID` together. `DISCORD_DEV_GUILD_ID` is optional and makes it a development bot: startup registers guild-scoped slash commands to that one guild (immediate, where global registration can take up to an hour to propagate) and every command is refused from any other guild. Leave it empty and startup registers commands globally instead, so the bot serves every server it has been added to and DMs. Switching from a dev guild to global would otherwise leave the old guild's commands registered alongside the global ones, and Discord renders both sets; startup clears any such leftovers so this resolves itself. `/menu`, `/wallet`, `/mint`, `/batch-mint`, `/task`, `/activity`, `/pnl`, `/gas`, `/sniper`, `/mode`, `/admin`, and `/link` use the same identity, validation, governance, transaction, scheduler, and storage services as Telegram. Replies are ephemeral; destructive and value-bearing commands require an explicit `confirm` option. Every sniper response labels its actual timing mode: safe/default after-confirmation copying or high-risk pending-mempool copying.
...
For a phase-aware OpenSea schedule, the selected UTC time is a **not-before wake-up**, not permission to broadcast blindly. The worker refreshes the drop, matches the persisted stage identity (UUID first, with a fail-closed legacy label/type fallback), and waits until that stage (or, in `earliest_eligible` mode, a later advertised stage) is actually live. OpenSea's wallet-specific mint builder then supplies the proof/signature-bearing call. An allowlist rejection advances the same durable task to the next advertised phase; the idempotency key is unchanged and phase waiting does not spend the normal RPC retry budget. A public SeaDrop still uses the fresh OpenSea phase UUID to distinguish repeated public phases, but its calldata, price, and live window are verified directly against the on-chain `PublicDrop` rather than depending on OpenSea's mint builder. Tasks stop at their persisted eligibility deadline (the known later phase window, capped at 24 hours from the selected start when created through current UI/bot flows).
...
### Configurable copy snipers

Wallet-copy snipers default to `confirmed` mode. The watcher records a matching source transaction, waits for the sniper's configured confirmation count, and verifies that the receipt still has the same block hash before submitting a copy. Duplicate block delivery and process restarts reuse the durable `(user, sniper, source transaction)` record, while reorg-dropped transactions are recorded as skipped rather than copied.

`pending` mode is an explicit, high-risk opt-in per sniper. It observes an address-filtered public pending-transaction stream and can submit before the source confirms. A dropped, replaced, or reverted source may therefore still leave the copied transaction spending gas or value. Enabling it requires a separate risk acknowledgement and a dedicated Alchemy WebSocket configured through the existing Railway sniper variables. The implemented and test-verified adapter is fail-closed to Ethereum and Polygon, the networks documented for that filtered subscription; a live deployed subscription acceptance check remains recorded in `docs/WORKLIST.md`. Other EVM chains remain confirmed-mode only until they have an equally narrow adapter. Sender-plus-nonce deduplication treats replacement hashes as one source action, and a bounded dispatcher isolates failures and provider bursts, but this mode cannot guarantee transaction ordering, inclusion, or front-running.
...
Existing sniper settings can be patched with `/updatesniper <id> <JSON>`. The merged configuration is fully validated before the database or running watcher is changed; unknown fields, malformed addresses, overlapping allow/deny lists, invalid limits, and non-boolean activation values are rejected. Moving an existing sniper into `pending` mode requires the same explicit risk acknowledgement.

The per-user creation default affects **new copy snipers only**; it never changes existing ones and does not alter manual or scheduled mint behavior. Set it from the Automation dashboard, Telegram `/sniperdefault confirmed|pending`, or Discord `/sniper default`. Selecting `pending` as the default requires the same warning confirmation and at least one configured pending-capable chain. Each new sniper may still override the default.
...
For local dashboard UI work, run Vite by itself. Its development proxy deliberately uses the
deployed Railway API so a second local process cannot claim real scheduler, sniper, or watcher
work from the shared database. The proxy is HTTPS, so Node must receive `--use-system-ca`; omitting
that flag produces a Vite `502` and the login screen can only report `Request failed`.

```powershell
# Preferred project launcher
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\project-npm.ps1 run dashboard:dev

# Exact Node-only equivalent on this machine
& 'C:\Users\General\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe' --use-system-ca .\node_modules\vite\bin\vite.js --config dashboard\vite.config.js
```

Do not also run `start` for ordinary dashboard styling/testing against the shared production data:
the application process owns durable workers, while Vite needs only the deployed API proxy.

For full-stack testing of unpushed backend changes, use an isolated database in `.env` and start a
dashboard-only API. This mode is accepted only in `development`/`test`; it disables bot transports,
reconciliation, schedulers, transaction bumping, social/retention workers, and sniper watchers so
test fixtures cannot execute automatically. Manual Mint, Send, and trigger-approval execution are
also rejected; previews and non-value-moving dashboard workflows remain available. It is not a
substitute for a later worker acceptance run.

```powershell
# Terminal 1 — isolated local API
$env:GHOSTMINT_DASHBOARD_ONLY='true'
& 'C:\Users\General\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe' --use-system-ca .\index.js

# Terminal 2 — local dashboard pointed at that API
$env:GHOSTMINT_DEV_API_TARGET='http://127.0.0.1:3000'
& 'C:\Users\General\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe' --use-system-ca .\node_modules\vite\bin\vite.js --config dashboard\vite.config.js
```

Unset those process-local variables, or open a fresh terminal, to return to the ordinary deployed-API
workflow. `GHOSTMINT_DASHBOARD_ONLY=true` is rejected when `NODE_ENV=production`.
...
Dashboard sessions use an eight-hour sliding idle timeout and a seven-day absolute lifetime. An account may have three active browser sessions; a fourth login revokes the least recently used session. The login screen reports whether a session ended because of inactivity, the absolute lifetime, explicit revocation, or the browser-session limit. Cookies are always `Secure` in production; local development deliberately omits that attribute so a phone testing the HTTP development server can retain its session. Private/incognito browsers may still clear cookies when closed.

Wallet receive surfaces show a QR containing only the public EVM address, including the one-time wallet-creation review. GhostMint never encodes a private key or recovery phrase in a QR. A recipient-address scanner is deferred until the dashboard has a real Send transaction flow; adding a scanner without a destination field would create a misleading control that cannot submit anything.
