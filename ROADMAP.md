...
- **15e — Telegram-only link-code generation:** Explicit product rule: Telegram is the only platform that can generate a link code; Discord and the dashboard may only consume one. Discord's `/link` `code` option is required (there is no bare `/link` on Discord), and its Settings menu button opens a modal to paste a Telegram-generated code rather than generating one. This closes an inconsistency from 15c, which had briefly given Discord's Settings menu the same generate-a-code button as Telegram's.
