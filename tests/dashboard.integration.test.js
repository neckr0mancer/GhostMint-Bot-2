...
integration('dashboard consumes the existing link code and persists revocable sessions',{timeout:120_000},async()=>{await runMigrations({connectionString:env.DATABASE_URL_UNPOOLED,migrationsDirectory:path.join(root,'migrations')});const pool=createDatabasePool({connectionString:env.DATABASE_URL,max:2});
...
integration('a link code generated through the shared bot command service can be consumed by a second platform to join the same account',{timeout:120_000},async()=>{
