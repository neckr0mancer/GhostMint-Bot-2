...
async function operationsServer(t){const sessions=new Map([['token-a',{userId:'user-a',csrfTokenHash:'csrf'}],['token-b',{userId:'user-b',csrfTokenHash:'csrf'}]]);const privateKey=`0x${'11'.repeat(32)}`;const recoveryPhrase='test test test test test test test test test test test junk';const calls=[];const selectedPresets=new Map();const commands={
