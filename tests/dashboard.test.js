...
test('a dashboard mint sends the final confirmed outcome to linked-platform notifications with the correct explorer',async t=>{const {base,notifications}=await operationsServer(t);const preview=await (await fetch(`${base}/api/mints/preview`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({walletLabel:'alpha'})})).json();await fetch(`${base}/api/mints/confirm`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({previewToken:preview.previewToken,confirmation:'CONFIRM'})});assert.equal(notifications.length,1);assert.equal(notifications[0].userId,'user-a');assert.match(notifications[0].message,/Mint confirmed on Robinhood Chain/);assert.match(notifications[0].message,/robinhoodchain\.blockscout\.com\/tx/);});
