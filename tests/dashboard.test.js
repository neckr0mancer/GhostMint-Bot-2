...
// Export rate limiting was REMOVED at the owner's request (2026-08-21). This fixture's limiter is
// 2/60s, so before that change a third export in one instance returned 429 -- this asserts it no
// longer does, which is the guard that the removal stays removed rather than silently regressing.
// The password check and the audit write are what still gate an export, and both are covered above.
test('keystore export is no longer rate limited, and every attempt still verifies the password',async t=>{const {base,calls}=await operationsServer(t);const attempt=()=>fetch(`${base}/api/wallets/alpha/export`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({securityPassword:'a-strong-enough-password',confirmation:'CONFIRM'})});for(let index=0;index<4;index+=1)assert.equal((await attempt()).status,200,`export ${index+1} of 4 should succeed with no limit`);const wrong=await fetch(`${base}/api/wallets/alpha/export`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({securityPassword:'not-the-password',confirmation:'CONFIRM'})});assert.equal(wrong.status,401,'removing the limiter must not weaken the password check itself');});
...
// fixture) and these tests exercise 'securitypassword', which STILL uses it. Only 'exportkey' had
// its limiter removed, so these still need one instance per attempt.
