...
// Windows may hand listen(0) one of the ports Fetch deliberately blocks (for example 6000).
// That makes an otherwise healthy HTTP fixture fail before a request reaches Express. Probe the
// selected URL once and rebind only for that specific Fetch policy error, keeping every test on an
// OS-assigned port without introducing a shared fixed-port race.
async function listenForFetch(server){
  for(let attempt=0;attempt<20;attempt+=1){
    await new Promise((resolve,reject)=>{const fail=error=>{server.off('listening',ready);reject(error);};const ready=()=>{server.off('error',fail);resolve();};server.once('error',fail);server.once('listening',ready);server.listen(0,'127.0.0.1');});
    const base=`http://127.0.0.1:${server.address().port}`;
    try{await fetch(base,{method:'HEAD'});return base;}
    catch(error){if(error?.cause?.message!=='bad port')throw error;await new Promise(resolve=>server.close(resolve));}
  }
  throw new Error('Could not allocate a Fetch-safe local test port');
}
...
async function appServer(t){const data=fixture();const api=createDashboardApi({auth:data.auth,identityRepository:{listLinkedAccounts:async()=>[{platform:'telegram',platformUserId:'42'}],getTheme:async()=>'ghost-mint',getDisplayName:async()=>null},loginRateLimiter:createCommandRateLimiter()});const app=express();app.use(express.json());app.use(api.securityHeaders);app.post('/api/auth/login',api.login);app.post('/api/auth/logout',api.requireSession,api.requireCsrf,api.logout);app.get('/api/profile',api.requireSession,api.profile);const server=http.createServer(app);const base=await listenForFetch(server);t.after(()=>new Promise(resolve=>server.close(resolve)));return {...data,base,server};}
...
test('requireSession refreshes the session cookie Max-Age on every authenticated request',async t=>{
  ...
  const server=http.createServer(app);const base=await listenForFetch(server);t.after(()=>new Promise(resolve=>server.close(resolve)));
...
async function operationsServer(t,options={}){const sessions=new Map([['token-a',{userId:'user-a',csrfTokenHash:'csrf'}],['token-b',{userId:'user-b',csrfTokenHash:'csrf'}]]);const privateKey=`0x${'11'.repeat(32)}`;const recoveryPhrase='test test test test test test test test test test test junk';const calls=[];const notifications=[];const errors=[];const selectedPresets=new Map();const defaultChains=new Map();const lowBalanceThresholds=new Map();const commands={
  ...
  createWalletWithRecoveryPhrase:async()=>({label:'new',address:'0x0000000000000000000000000000000000000002',chain:'ethereum',privateKey,recoveryPhrase}),
  ...
  mintPresets:async()=>[],prepareMint:async(userId,input)=>{if(input.walletLabel==='already-minted')throw new ValidationError({field:'quantity',message:'this wallet has reached the mint limit'});if(input.walletLabel==='low-balance'){const error=new TransactionSafetyError('INSUFFICIENT_BALANCE','Wallet balance is below the estimated transaction cost',{balanceWei:2n,estimatedGasCostWei:3n,estimatedCostWei:5n});error.previewContext={wallet:{label:'low-balance',address:'0x0000000000000000000000000000000000000002',chain:'ethereum'},chain:'robinhood',preview:{contractAddress:'0x0000000000000000000000000000000000000003',methodSignature:'mint(uint256)',nativeValueWei:'2',nativeValue:'0.000000000000000002',arguments:[{name:'quantity',value:'1'}]}};throw error;}return {wallet:{label:input.walletLabel,address:'0x0000000000000000000000000000000000000001',chain:'ethereum'},prepared:{chain:'robinhood',preview:{contractAddress:'0x0000000000000000000000000000000000000003',methodSignature:'mint(uint256)',nativeValueWei:'0',nativeValue:'0.0',arguments:[{name:'quantity',value:'1'}]}},simulation:{simulationEnabled:false,simulationPerformed:true,simulationPassed:true,gasLimit:21000n,balanceWei:10n,estimatedGasCostWei:1n,estimatedCostWei:1n}};},
  submitPreparedMint:options.submitPreparedMint||(async(userId,value)=>{const label=value.wallet.label;if(label==='broken')throw new ValidationError({field:'walletLabel',message:'insufficient funds'});if(label==='broadcast-unknown')throw new TransactionSafetyError('BROADCAST_UNKNOWN','Transaction broadcast outcome is unknown; reconciliation will continue');calls.push(['mint',userId,value.simulation.simulationPassed]);const outcomes={reverted:{state:'reverted',txHash:`0x${'cd'.repeat(32)}`,reason:'receipt status was 0'},unknown:{state:'unknown',txHash:`0x${'ef'.repeat(32)}`,reason:'confirmation timeout elapsed'},pending:{state:'pending',txHash:`0x${'12'.repeat(32)}`},submitted:{state:'submitted',txHash:`0x${'34'.repeat(32)}`},replaced:{state:'replaced',txHash:`0x${'56'.repeat(32)}`}};return outcomes[label]||{state:'confirmed',txHash:`0x${'ab'.repeat(32)}`};}),
 ...
 const api=createDashboardApi({auth,commands,supportedChains:['ethereum','robinhood'],identityRepository:{listLinkedAccounts:async()=>[],getTheme:async userId=>themes.get(userId)||'ghost-mint',setTheme:async(userId,theme)=>{themes.set(userId,theme);return theme;},getDisplayName:async()=>null,getDefaultChain:async userId=>defaultChains.get(userId)||null,setDefaultChain:async(userId,value)=>{defaultChains.set(userId,value);return value;},getLowBalanceThreshold:async userId=>lowBalanceThresholds.get(userId)||'0.01',setLowBalanceThreshold:async(userId,value)=>{lowBalanceThresholds.set(userId,value);return value;},getSecurityPasswordHash:async userId=>securityPasswordHashes.get(userId)||null,setSecurityPasswordHash:async(userId,hash)=>{securityPasswordHashes.set(userId,hash);},
 ...
 },notifyUser:options.notifyUser||(async(userId,message)=>{if(options.notificationFailure)throw new Error('notification unavailable');notifications.push({userId,message});}),log:value=>errors.push(value),chains:{robinhood:{name:'Robinhood Chain',ex:'https://robinhoodchain.blockscout.com/tx/'}},loginRateLimiter:createCommandRateLimiter(),passwordLoginRateLimiter:createCommandRateLimiter({limit:2,windowMs:60_000}),exportKeyRateLimiter:createCommandRateLimiter({limit:2,windowMs:60_000})});const app=express();app.use(express.json());app.use(api.securityHeaders);mountDashboardRoutes(app,api);app.use(api.error);const server=http.createServer(app);const base=await listenForFetch(server);t.after(()=>new Promise(resolve=>server.close(resolve)));return {base,calls,notifications,errors,privateKey,recoveryPhrase,securityPasswordHashes,usernames};}
...
test('schedule conflicts keep their domain code and safe allowance details',async t=>{
  const {base}=await operationsServer(t,{createTask:async()=>{
    throw new ValidationError({field:'quantity',message:'this wallet can schedule 1 more mint'},
      'SCHEDULE_ALLOWANCE_EXCEEDED','Request validation failed',
      {remaining:'1',maximum:'10',minted:'2',reserved:'7'});
  }});
  const response=await fetch(`${base}/api/tasks`,{method:'POST',headers:authHeaders('a',true),body:'{}'});
...
test('unexpected schedule failures are traceable without leaking the raw database error',async t=>{
  ...
  const {base,errors}=await operationsServer(t,{createTask:async()=>{throw Object.assign(new Error(secret),{
    code:'23502',table:'mint_tasks',column:'preflight_target_at'});}});
  const response=await fetch(`${base}/api/tasks`,{method:'POST',headers:authHeaders('a',true),body:'{}'});
  assert.equal(response.status,500);
  const body=await response.json();
  assert.equal(body.error,'This request could not be completed.');
  assert.equal(body.code,'INTERNAL_ERROR');
  assert.match(body.requestId,/^[a-f0-9]{12}$/);
  assert.equal(JSON.stringify(body).includes(secret),false);
  assert.equal(errors.length,1);
  assert.match(errors[0],/23502/);
  assert.match(errors[0],/preflight_target_at/);
  assert.equal(errors[0].includes(secret),false);
});
test('wallet creation returns only its one-time phrase while ordinary responses expose no secrets',async t=>{const {base,privateKey,recoveryPhrase}=await operationsServer(t);const created=await fetch(`${base}/api/wallets/create`,{method:'POST',headers:authHeaders('a',true),body:'{}'});assert.equal(created.status,201);assert.match(created.headers.get('cache-control'),/no-store/);const createdText=await created.text();assert.equal(createdText.includes(privateKey),false);assert.equal(JSON.parse(createdText).recoveryPhrase,recoveryPhrase);const listedText=await (await fetch(`${base}/api/wallets`,{headers:authHeaders('a')})).text();assert.equal(listedText.includes(recoveryPhrase),false,'the one-time phrase must never appear in later wallet reads');const body=JSON.parse(listedText);assert.deepEqual(Object.keys(body[0]).sort(),['addedAt','address','balances','chain','label','minted'],'publicWallet projects exactly these fields -- addedAt is needed by the wallet details overlay, and nothing from the key envelope or creation phrase may ever join them');const failed=await fetch(`${base}/api/wallets/import`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({privateKey})});assert.equal(failed.status,500);assert.equal((await failed.text()).includes(privateKey),false);});
...
// Five wallets must be exportable in one backup session. Only incorrect password guesses consume
// the failure bucket; even after that bucket returns 429, supplying the correct password succeeds.
test('correct-password exports have no wallet-count ceiling while wrong guesses are throttled',async t=>{const {base}=await operationsServer(t);const attempt=password=>fetch(`${base}/api/wallets/alpha/export`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({securityPassword:password,confirmation:'CONFIRM'})});for(let index=0;index<5;index+=1)assert.equal((await attempt('a-strong-enough-password')).status,200);assert.equal((await attempt('wrong-password-value')).status,401);assert.equal((await attempt('another-wrong-value')).status,401);const limited=await attempt('third-wrong-password');assert.equal(limited.status,429);assert.equal(limited.headers.get('retry-after'),'60');assert.equal((await limited.json()).error,'Too many incorrect password attempts');assert.equal((await attempt('a-strong-enough-password')).status,200,'the failure throttle must not lock out a correct password');});
// Separate operationsServer(t) instances keep the independently tested security-password change
// scenarios from sharing this fixture's deliberately tiny 2/60s limiter.
...
test('mint preview returns JSON-safe balance diagnostics for an insufficient wallet',async t=>{const {base}=await operationsServer(t);
  const success=await (await fetch(`${base}/api/mints/preview`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({walletLabel:'alpha'})})).json();
  assert.deepEqual(success.items[0].simulation.balanceWei,'10');
  assert.deepEqual(success.items[0].simulation.estimatedGasCostWei,'1');
  const response=await fetch(`${base}/api/mints/preview`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({walletLabel:'low-balance'})});
  assert.equal(response.status,400);
  assert.deepEqual(await response.json(),{error:'Wallet balance is below the estimated transaction cost',code:'INSUFFICIENT_BALANCE',details:{balanceWei:'2',estimatedGasCostWei:'3',estimatedCostWei:'5'}});
});

test('a dashboard mint sends an explicit final-success outcome to linked-platform notifications with the correct explorer',async t=>{const {base,notifications}=await operationsServer(t);const preview=await (await fetch(`${base}/api/mints/preview`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({walletLabel:'alpha'})})).json();const response=await fetch(`${base}/api/mints/confirm`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({previewToken:preview.previewToken,confirmation:'CONFIRM'})});const body=await response.json();assert.equal(body.results[0].outcome,'confirmed');assert.equal(notifications.length,1);assert.equal(notifications[0].userId,'user-a');assert.match(notifications[0].message,/Mint successful/);assert.match(notifications[0].message,/transaction was confirmed on Robinhood Chain/);assert.match(notifications[0].message,/robinhoodchain\.blockscout\.com\/tx/);});
...
test('one wallet failing batch simulation is skipped while eligible wallets still receive a confirmable preview',async t=>{const {base,calls}=await operationsServer(t);const response=await fetch(`${base}/api/mints/preview`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({walletLabels:['already-minted','fresh']})});assert.equal(response.status,200);const preview=await response.json();assert.ok(preview.previewToken);assert.deepEqual(preview.items.map(item=>item.wallet.label),['fresh']);assert.deepEqual(preview.failures,[{walletLabel:'already-minted',error:'quantity this wallet has reached the mint limit',code:'VALIDATION_ERROR'}]);const confirm=await fetch(`${base}/api/mints/confirm`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({previewToken:preview.previewToken,confirmation:'CONFIRM'})});assert.equal(confirm.status,202);assert.deepEqual((await confirm.json()).results.map(item=>[item.label,item.status]),[['fresh','success']]);assert.deepEqual(calls.filter(call=>call[0]==='mint').map(call=>call[1]),['user-a']);});
test('a failed batch wallet retains only safe preview diagnostics while another stays confirmable',async t=>{const {base,calls}=await operationsServer(t);const response=await fetch(`${base}/api/mints/preview`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({walletLabels:['low-balance','fresh']})});assert.equal(response.status,200);const preview=await response.json();assert.ok(preview.previewToken);assert.equal(preview.items[0].wallet.label,'fresh');assert.equal(preview.items[0].chain,'robinhood');assert.deepEqual(preview.failures,[{walletLabel:'low-balance',error:'Wallet balance is below the estimated transaction cost',code:'INSUFFICIENT_BALANCE',details:{balanceWei:'2',estimatedGasCostWei:'3',estimatedCostWei:'5'},wallet:{label:'low-balance',address:'0x0000000000000000000000000000000000000002',chain:'ethereum'},chain:'robinhood',preview:{contractAddress:'0x0000000000000000000000000000000000000003',methodSignature:'mint(uint256)',nativeValueWei:'2',nativeValue:'0.000000000000000002',arguments:[{name:'quantity',value:'1'}]}}]);const confirm=await fetch(`${base}/api/mints/confirm`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({previewToken:preview.previewToken,confirmation:'CONFIRM'})});assert.equal(confirm.status,202);assert.deepEqual((await confirm.json()).results.map(item=>[item.label,item.status]),[['fresh','success']]);assert.deepEqual(calls.filter(call=>call[0]==='mint').map(call=>call[1]),['user-a']);});
test('an all-failed batch simulation returns safe per-wallet results and no usable preview token',async t=>{const {base}=await operationsServer(t);const response=await fetch(`${base}/api/mints/preview`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({walletLabels:['already-minted']})});assert.equal(response.status,200);const preview=await response.json();assert.equal(preview.previewToken,null);assert.deepEqual(preview.items,[]);assert.equal(preview.failures.length,1);const confirm=await fetch(`${base}/api/mints/confirm`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({previewToken:preview.previewToken,confirmation:'CONFIRM'})});assert.equal(confirm.status,400);});
...
test('a failure on one wallet in a batch confirmation does not cancel the others, and reports its own reason',async t=>{const {base,calls,notifications}=await operationsServer(t);const preview=await (await fetch(`${base}/api/mints/preview`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({walletLabels:['alpha','broken']})})).json();assert.equal(preview.items.length,2);const confirm=await fetch(`${base}/api/mints/confirm`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({previewToken:preview.previewToken,confirmation:'CONFIRM'})});assert.equal(confirm.status,202);const body=await confirm.json();assert.deepEqual(body.results.map(entry=>entry.label),['alpha','broken']);const [alphaResult,brokenResult]=body.results;assert.equal(alphaResult.status,'success');assert.equal(alphaResult.outcome,'confirmed');assert.equal(brokenResult.status,'failed');assert.equal(brokenResult.outcome,'failed_prebroadcast');assert.equal(brokenResult.error,'walletLabel insufficient funds');assert.deepEqual(calls.filter(call=>call[0]==='mint').map(call=>call[1]),['user-a']);assert.equal(notifications.length,2);assert.match(notifications[1].message,/Mint not submitted/);assert.match(notifications[1].message,/stopped before broadcast on Robinhood Chain/);assert.match(notifications[1].message,/insufficient funds/);assert.doesNotMatch(notifications[1].message,/Nothing was minted/);});

test('batch submissions start independently and a hanging notification cannot block another wallet or the response',{timeout:2_000},async t=>{
  let markSecondStarted;const secondStarted=new Promise(resolve=>{markSecondStarted=resolve;});const started=[];
  const submitPreparedMint=async(_userId,value)=>{const label=value.wallet.label;started.push(label);
    if(label==='alpha')await Promise.race([secondStarted,new Promise((_,reject)=>setTimeout(()=>reject(new Error('second wallet did not start concurrently')),300))]);
    else markSecondStarted();
    return {state:'confirmed',txHash:`0x${(label==='alpha'?'aa':'bb').repeat(32)}`};};
  const {base}=await operationsServer(t,{submitPreparedMint,notifyUser:()=>new Promise(()=>{})});
  const preview=await (await fetch(`${base}/api/mints/preview`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({walletLabels:['alpha','alpha-2']})})).json();
  const response=await fetch(`${base}/api/mints/confirm`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({previewToken:preview.previewToken,confirmation:'CONFIRM'})});
  assert.equal(response.status,202);const body=await response.json();
  assert.deepEqual(started,['alpha','alpha-2']);
  assert.deepEqual(body.results.map(item=>[item.label,item.chain,item.outcome]),[
    ['alpha','robinhood','confirmed'],['alpha-2','robinhood','confirmed'],
  ]);
  assert.equal('error' in body.results[0],false);
});

test('a confirmed intent remains confirmed if a later adapter error carries that chain outcome',async t=>{
  const confirmed={state:'confirmed',txHash:`0x${'ac'.repeat(32)}`};
  const submitPreparedMint=async()=>{const error=new Error('activity write failed');error.transactionOutcome={intent:confirmed};throw error;};
  const {base}=await operationsServer(t,{submitPreparedMint});
  const preview=await (await fetch(`${base}/api/mints/preview`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({walletLabel:'alpha'})})).json();
  const response=await fetch(`${base}/api/mints/confirm`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({previewToken:preview.previewToken,confirmation:'CONFIRM'})});
  assert.equal(response.status,202);const [result]=(await response.json()).results;
  assert.equal(result.outcome,'confirmed');assert.equal(result.status,'success');assert.equal(result.result.txHash,confirmed.txHash);
  assert.equal('error' in result,false);
});

test('mint confirmation distinguishes every transaction outcome without false no-broadcast claims',async t=>{const {base,notifications}=await operationsServer(t);const labels=['alpha','reverted','unknown','pending','submitted','replaced','broadcast-unknown','broken'];const preview=await (await fetch(`${base}/api/mints/preview`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({walletLabels:labels})})).json();const response=await fetch(`${base}/api/mints/confirm`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({previewToken:preview.previewToken,confirmation:'CONFIRM'})});assert.equal(response.status,202);const body=await response.json();assert.deepEqual(body.results.map(item=>[item.label,item.status,item.outcome]),[
  ['alpha','success','confirmed'],['reverted','failed','reverted'],['unknown','uncertain','unknown'],['pending','pending','pending'],['submitted','pending','submitted'],['replaced','failed','replaced'],['broadcast-unknown','uncertain','unknown'],['broken','failed','failed_prebroadcast'],
]);const byLabel=Object.fromEntries(notifications.map(item=>[labels[notifications.indexOf(item)],item.message]));assert.match(byLabel.reverted,/Mint reverted/);assert.match(byLabel.reverted,/was broadcast/);assert.match(byLabel.unknown,/Mint outcome unknown/);assert.match(byLabel.unknown,/may have been broadcast/);assert.match(byLabel.pending,/Mint pending/);assert.match(byLabel.pending,/was broadcast/);assert.match(byLabel.submitted,/Mint submitted/);assert.match(byLabel.submitted,/not yet confirmed/);assert.match(byLabel.replaced,/Mint replaced/);assert.match(byLabel['broadcast-unknown'],/may have been broadcast/);assert.match(byLabel.broken,/stopped before broadcast/);for(const item of notifications)assert.doesNotMatch(item.message,/Nothing was minted/);
});

test('notification delivery failure cannot change a confirmed mint response',async t=>{const {base,notifications}=await operationsServer(t,{notificationFailure:true});const preview=await (await fetch(`${base}/api/mints/preview`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({walletLabel:'alpha'})})).json();const response=await fetch(`${base}/api/mints/confirm`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({previewToken:preview.previewToken,confirmation:'CONFIRM'})});assert.equal(response.status,202);assert.deepEqual((await response.json()).results.map(item=>[item.status,item.outcome]),[['success','confirmed']]);assert.deepEqual(notifications,[]);});
...
test('profile reports the account dashboard theme and display preferences',async t=>{const {base}=await operationsServer(t);const profile=await (await fetch(`${base}/api/profile`,{headers:authHeaders('a')})).json();assert.equal(profile.theme,'ghost-mint');assert.equal(profile.defaultChain,null);assert.equal(profile.lowBalanceThreshold,'0.01');});
...
test('default-chain updates accept configured Robinhood Chain, reject unsupported chains, and persist per user',async t=>{const {base}=await operationsServer(t);
  const noCsrf=await fetch(`${base}/api/profile/default-chain`,{method:'PUT',headers:{cookie:'ghostmint_session=token-a','content-type':'application/json'},body:JSON.stringify({defaultChain:'robinhood'})});
  assert.equal(noCsrf.status,403);
  const unsupported=await fetch(`${base}/api/profile/default-chain`,{method:'PUT',headers:authHeaders('a',true),body:JSON.stringify({defaultChain:'fantom'})});
  assert.equal(unsupported.status,400);
  const updated=await fetch(`${base}/api/profile/default-chain`,{method:'PUT',headers:authHeaders('a',true),body:JSON.stringify({defaultChain:'robinhood'})});
  assert.equal(updated.status,200);assert.deepEqual(await updated.json(),{defaultChain:'robinhood'});
  assert.equal((await (await fetch(`${base}/api/profile`,{headers:authHeaders('a')})).json()).defaultChain,'robinhood');
  assert.equal((await (await fetch(`${base}/api/profile`,{headers:authHeaders('b')})).json()).defaultChain,null);
});
test('low-balance warning choices are validated, CSRF-protected, and persisted per user',async t=>{const {base}=await operationsServer(t);
  const noCsrf=await fetch(`${base}/api/profile/low-balance-threshold`,{method:'PUT',headers:{cookie:'ghostmint_session=token-a','content-type':'application/json'},body:JSON.stringify({lowBalanceThreshold:'0.025'})});
  assert.equal(noCsrf.status,403);
  const arbitrary=await fetch(`${base}/api/profile/low-balance-threshold`,{method:'PUT',headers:authHeaders('a',true),body:JSON.stringify({lowBalanceThreshold:'0.037'})});
  assert.equal(arbitrary.status,400);
  const updated=await fetch(`${base}/api/profile/low-balance-threshold`,{method:'PUT',headers:authHeaders('a',true),body:JSON.stringify({lowBalanceThreshold:'0.025'})});
  assert.equal(updated.status,200);assert.deepEqual(await updated.json(),{lowBalanceThreshold:'0.025'});
...
test('an out-of-range page or pageSize is a 400 naming the field, not a 500',async t=>{
  ...
  const server=http.createServer(app);const base=await listenForFetch(server);
  t.after(()=>new Promise(resolve=>server.close(resolve)));
  const headers={cookie:'ghostmint_session=token-a'};
