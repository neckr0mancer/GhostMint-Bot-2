...
async function operationsServer(t){const sessions=new Map([['token-a',{userId:'user-a',csrfTokenHash:'csrf'}],['token-b',{userId:'user-b',csrfTokenHash:'csrf'}]]);const privateKey=`0x${'11'.repeat(32)}`;const recoveryPhrase='test test test test test test test test test test test junk';const calls=[];const notifications=[];const selectedPresets=new Map();const commands={
...
test('a dashboard mint sends an explicit final-success outcome to linked-platform notifications with the correct explorer',async t=>{const {base,notifications}=await operationsServer(t);const preview=await (await fetch(`${base}/api/mints/preview`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({walletLabel:'alpha'})})).json();await fetch(`${base}/api/mints/confirm`,{method:'POST',headers:authHeaders('a',true),body:JSON.stringify({previewToken:preview.previewToken,confirmation:'CONFIRM'})});assert.equal(notifications.length,1);assert.equal(notifications[0].userId,'user-a');assert.match(notifications[0].message,/Mint successful/);assert.match(notifications[0].message,/transaction was confirmed on Robinhood Chain/);assert.match(notifications[0].message,/robinhoodchain\.blockscout\.com\/tx/);});
